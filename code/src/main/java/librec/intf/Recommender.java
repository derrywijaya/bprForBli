...
public abstract class Recommender implements Runnable {
	 ...
	 */
	protected Map<Measure, Double> computeScores() throws Exception {
		...
		String translationPath = cf.getPath("dataset.tocomputetrain");
		Logs.debug("Tobecomputed path: " + translationPath);
		BufferedReader br = FileIO.getReader(translationPath);
		String line;
		Map<Integer,String> eids = new HashMap<Integer,String>();
		Map<Integer,String> fids = new HashMap<Integer,String>();
		while ((line = br.readLine()) != null) {
			String temp0[] = line.split("\t");
			String s = line.trim().toLowerCase();
			s = s.replace(",", "_").trim();
			String temp[] = s.split("\t");
			String eword = "row-" + temp[1].trim();
			String fword = "column-" + temp[0].trim();
			if (rateDao.getUserIds().get(eword)!=null) {
				if (rateDao.getItemIds().get(fword)!=null) {
					int eid = rateDao.getUserId(eword);
					int fid = rateDao.getItemId(fword);
					eids.put(eid, temp0[1].trim());
					fids.put(fid, temp0[0].trim());
				}
			}
		}
		br.close();
		for (Map.Entry<Integer, String> e1 : eids.entrySet()) {
			int eid = e1.getKey();
			String estring = e1.getValue();
			for (Map.Entry<Integer, String> e2 : fids.entrySet()) {
				int fid = e2.getKey();
				String fstring = e2.getValue();
				final double rank = ranking(eid, fid);
				if (!Double.isNaN(rank)) {
					System.out.println("JTRAIN\t" + fstring + "\t" + estring + "\t" + rank );
				}
			}
		}
		
		translationPath = cf.getPath("dataset.tocomputedev");
		Logs.debug("Tobecomputed path: " + translationPath);
		br = FileIO.getReader(translationPath);
		eids.clear();
		fids.clear();
		while ((line = br.readLine()) != null) {
			String temp0[] = line.split("\t");
		...
		for (Map.Entry<Integer, String> e1 : eids.entrySet()) {
			...
			for (Map.Entry<Integer, String> e2 : fids.entrySet()) {
				...
				final double rank = ranking(eid, fid);
				if (!Double.isNaN(rank)) {
					System.out.println("JDEV\t" + fstring + "\t" + estring + "\t" + rank );
				}
			}
		}
		
		translationPath = cf.getPath("dataset.tocomputetest");
		Logs.debug("Tobecomputed path: " + translationPath);
		br = FileIO.getReader(translationPath);
		eids.clear();
		fids.clear();
		...
		for (Map.Entry<Integer, String> e1 : eids.entrySet()) {
			...
			for (Map.Entry<Integer, String> e2 : fids.entrySet()) {
				...
				final double rank = ranking(eid, fid);
				if (!Double.isNaN(rank)) {
					System.out.println("JTEST\t" + fstring + "\t" + estring + "\t" + rank );
				}
			}
		}
		
		translationPath = cf.getPath("dataset.tocomputemlp");
		Logs.debug("Tobecomputed path: " + translationPath);
		br = FileIO.getReader(translationPath);
		eids.clear();
		fids.clear();
		...
		for (Map.Entry<Integer, String> e1 : eids.entrySet()) {
			...
			for (Map.Entry<Integer, String> e2 : fids.entrySet()) {
				...
				final double rank = ranking(eid, fid);
				if (!Double.isNaN(rank)) {
					System.out.println("JMLP\t" + fstring + "\t" + estring + "\t" + rank );
				}
			}
		}

		/*Map<Integer, Map<Integer, Integer>> testset = new HashMap<Integer, Map<Integer, Integer>>();
		Map<Integer, Map<String, String>> testsetstring = new HashMap<Integer, Map<String, String>>();
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			for (int i : testItems) {
				Map<Integer, Integer> map = testset.get(i);
				if (map == null) map = new HashMap<Integer, Integer>();
				map.put(u,u);
				testset.put(i, map);
				
				Map<String, String> mapstring = testsetstring.get(i);
				if (mapstring == null) mapstring = new HashMap<String, String>();
				String userstring = rateDao.getUserId(u);
				mapstring.put(userstring, userstring);
				testsetstring.put(i, mapstring);
			}
		}
		for (int u = 0, um = trainMatrix.numRows(); u < um; u++) {
			List<Integer> testItems = trainMatrix.getColumns(u);
			if (testItems == null || testItems.size() == 0) continue;
			for (int i : testItems) {
				Map<Integer, Integer> map = testset.get(i);
				if (map == null) map = new HashMap<Integer, Integer>();
		...
		/*for (Map.Entry<Integer, Map<Integer,Integer>> e : testset.entrySet()) {
			int testitem = e.getKey();
			String stringitem = rateDao.getItemId(testitem);
			if (!stringitem.startsWith("column-")) continue;
			System.out.println("processing\t" + stringitem);
			Map<Integer, Integer> correctusers = e.getValue();
			//List<Map.Entry<Integer, Double>> userScores = new ArrayList<>(Lists.initSize(rateDao.numUsers()));
			for (int v = 0, vm = rateDao.numUsers(); v < vm; v++) {
				//if (correctusers.get(v)==null) continue;
				String uid = rateDao.getUserId(v);
				if (!uid.startsWith("row-")) continue;
				final double rank = ranking(v, testitem);
				if (!Double.isNaN(rank)) {
					System.out.println("JSCORES\t" + stringitem + "\t" + uid + "\t" + rank );
					//userScores.add(new SimpleImmutableEntry<Integer, Double>(v, rank));
				}
			}
			...
			if (isResultsOut) {
				...
				preds.add(testitem + ": " + rateDao.getItemId(testitem) + ": " + sb.toString());
				derrytotal++;
				if (preds.size() >= 1000) {
		...
		/*if (isResultsOut && preds.size() > 0) {
			...
			double derryaccuracymt = (double)derrycorrectmt / (double)derrytotal;
			Logs.debug("OTSaccuracy@10: " + derryaccuracymt + " : " + derrycorrectmt + " correct of " + derrytotal);
		}*/
	...
	protected Map<Measure, Double> evalRatings() throws Exception {
		...
		for (MatrixEntry me : testMatrix) {
			...
			if (isResultsOut) {
				...
				if (j < rateDao.numItems()-1) {
					double pred1 = predict(u, j+1, true);
					if (!Double.isNaN(pred1)) {
						preds.add(rateDao.getUserId(u) + " " + rateDao.getItemId(j+1) + " " + 0.0 + " " + (float)pred1 );
					}
				}
				if (preds.size() >= 1000) {
					FileIO.writeList(toFile, preds, true);
	...
	protected Map<Measure, Double> evalRankings() throws Exception {
		...
		// start edit
		String translationPath = cf.getPath("dataset.translation");
		Logs.debug("Translation path: " + translationPath);
		BufferedReader br = FileIO.getReader(translationPath);
		String line;
		Map<String, String>lemmas = new HashMap<String, String>();
		int indx = 0;
		while ((line = br.readLine()) != null) {
			if (indx % 1000 == 0) System.out.println("LEMMAS\t" + line);
			indx++;
			String temp[] = line.split("\t");
			String word = temp[0].trim();
			String lemma = "";
			for (int i = 1; i < temp.length; i++) {
				String l = temp[i].trim();
				String temporary[] = l.split(" \\|\\|\\| ");
				String lem = temporary[0].trim();
				lemma = lemma + lem + " ";
			}
			lemma = lemma.trim();
		...
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			for (int i : testItems) {
				...
				String userstring = rateDao.getUserId(u);
				mapstring.put(userstring, userstring);
				String lemma = lemmas.get(userstring);
				if (lemma != null) mapstring.put(lemma, lemma);
				testsetstring.put(i, mapstring);
			}
		}
		int idx = 0;
		for (Map.Entry<Integer, Map<Integer,Integer>> e : testset.entrySet()) {
			idx++;
			int testitem = e.getKey();
			System.out.println("processing\t" + rateDao.getItemId(testitem));
			Map<Integer, Integer> correctusers = e.getValue();
			Map<String, String> correctuserstrings = testsetstring.get(testitem);
			List<Map.Entry<Integer, Double>> userScores = new ArrayList<>(Lists.initSize(rateDao.numUsers()));
			for (int v = 0, vm = rateDao.numUsers(); v < vm; v++) {
				String uid = rateDao.getUserId(v);
				if (!uid.startsWith("row-")) continue;
				List<Integer> ratedItems = trainMatrix.getColumns(v);
				if (!algoName.endsWith("Extended")) { // if algorithm name ends with 'Extended', you are NOT doing evaluation. you are using the train+test to annotate all foreign words with their candidate english words
					if (ratedItems.contains(testitem)) continue; 					
				}
				final double rank = ranking(v, testitem);
				if (!Double.isNaN(rank)) {
					userScores.add(new SimpleImmutableEntry<Integer, Double>(v, rank));
			...
			for (Map.Entry<Integer, Double> kv : recomd) {
				...
				if (isResultsOut && count < 100) {
					// restore back to the original item id
					sb.append("(").append(userstring);
					boolean lemmacorrect = false;
					if (lemma != null) {
						lemmacorrect = correctuserstrings.containsKey(lemma);
					}
					if (correctusers.get(user)!=null || lemmacorrect) {
						sb.append("**"); // indicating correct recommendation
						if (!found) {
							derrycorrectmt++;	
							System.out.println("correct\t" + derrycorrectmt + " of " + idx);
							found=true;
						} 
					} 
					sb.append(", ").append(kv.getValue().floatValue()).append(")");
		...
		for (Map.Entry<Integer, Map<Integer,Integer>> e : testset.entrySet()) {
			int testitem = e.getKey();
			Map<Integer, Integer> correctusers = e.getValue();
			String itemstring = rateDao.getItemId(testitem);
			Map<String,String> candidateusers = translations.get(itemstring);
			if (candidateusers == null) candidateusers = new HashMap<String,String>();
			List<Map.Entry<Integer, Double>> userScores = new ArrayList<>(Lists.initSize(rateDao.numUsers()));
			for (int v = 0, vm = rateDao.numUsers(); v < vm; v++) {
				String uid = rateDao.getUserId(v);
				if (candidateusers.size()>0 && candidateusers.get(uid)==null)continue;
				if (!uid.startsWith("row-")) continue;
				List<Integer> ratedItems = trainMatrix.getColumns(v);
				if (ratedItems.contains(testitem)) continue;
				final double rank = ranking(v, testitem);
		...
		if (isResultsOut && preds.size() > 0) {
			...
			Logs.debug("{}{} has writeen item recommendations to {}", algoName, foldInfo, toFile);
			double derryaccuracymt = (double)derrycorrectmt / (double)derrytotal;
			Logs.debug("filteredOTSaccuracy@10: " + derryaccuracymt + " : " + derrycorrectmt + " correct of " + derrytotal);
		...
		ArrayList<String> dusers = new ArrayList<String>();
		Logs.debug("number of all users\t" + testMatrix.numRows());
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			String du = rateDao.getUserId(u);
			if (du.startsWith("row-")) {
				dusers.add(du);				
			}
		}
		...
		for (int u = 0, um = testMatrix.numRows(); u < um; u++) {
			...
			for (Map.Entry<Integer, Double> kv : recomd) {
				...
				String ditem = rateDao.getItemId(item);
				ditem = "row-" + ditem.replaceFirst("column-", "");
				if (!dusers.contains(ditem)) continue;
			...
			if (isResultsOut) {
				// restore back to the original user id
				preds.add(u + ": " + rateDao.getUserId(u) + ": " + sb.toString());
				derrytotal++;
				if (preds.size() >= 1000) {
					FileIO.writeList(toFile, preds, true);
					preds.clear();
		...
		if (isResultsOut && preds.size() > 0) {
			FileIO.writeList(toFile, preds, true);
			Logs.debug("{}{} has writeen item recommendations to {}", algoName, foldInfo, toFile);
			double derryaccuracy = (double)derrycorrect / (double)derrytotal;
			Logs.debug("accuracy@10: " + derryaccuracy + " : " + derrycorrect + " correct of " + derrytotal);
		}*/
