...
public abstract class IterativeRecommender extends Recommender {
	...
	protected boolean isConverged(int iter) throws Exception {
		...
		boolean cond1 = Math.abs(delta_loss) < 10;
	...
	protected void saveModel() throws Exception {
		...
		FileIO.serialize(trainMatrix.toString(), dirPath + "trainMatrix" + suffix);
		FileIO.serialize(testMatrix.toString(), dirPath + "testMatrix" + suffix);

		// write matrices P, Q
		String pstring = "";
		for (int u = 0; u < P.numRows(); u++) {
			String s = rateDao.getUserId(u) + ":" + P.row(u).toString();
			pstring = pstring + s + "\n";
		}
		pstring = pstring.trim();
		FileIO.serialize(pstring, dirPath + "userFactors" + suffix);
		//FileIO.serialize(P.toString(), dirPath + "userFactors" + suffix);
		pstring = "";
		for (int u = 0; u < Q.numRows(); u++) {
			String s = rateDao.getItemId(u) + ":" + Q.row(u).toString();
			pstring = pstring + s + "\n";
		}
		pstring = pstring.trim();
		FileIO.serialize(pstring, dirPath + "itemFactors" + suffix);
		//FileIO.serialize(Q.toString(), dirPath + "itemFactors" + suffix);
